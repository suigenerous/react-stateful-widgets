...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList.find(programmer => programmer.id === featuredId).name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredId != null ? 'yellow' :'royalblue', // 🤔
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => { /* set the featured id passing dev.id */ setFeaturedId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
